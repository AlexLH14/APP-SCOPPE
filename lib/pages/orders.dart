                    ...
                    child: ListView(
                        children: (snapshot.data!.docs.isNotEmpty)
                                ...
                                .map((e) => Card(
                                    ...
                                    child: ListTile(
                                      leading: Icon(Icons.article_outlined,
                                          ...
                                          size: 45),
                                      title: Text(
                                          "${e['cadena']['nombre'].toString()} | ${e['local']['nombre'].toString()}"),
                                      subtitle: Text(
                                          "${e['mercaderista']['nombre'].toString()} | Hoy${e['visita'].substring(10)}"),
                                      onTap: () async {
                                        bool permitido =
                                            await ensureLocationEnabled(
                                                context);
                                        if (!context.mounted)
                                          return; // <- línea recomendada
                                        if (!permitido) return;
                            ...
                            : <Widget>[
                                ...
                                Center(
                                    child: Text(
                                        "No existen ordenes! ${DateFormat('yyyy-MM-dd HH:mm').format(DateTime.now())} ${(DateTime.parse("2023-09-18 01:00").compareTo(DateTime.now()))}",
                                ...
                                .map((e) => Card(
                                    ...
                                    child: ListTile(
                                      ...
                                      title: Text(
                                          "${e['cadena']['nombre'].toString()} | ${e['local']['nombre'].toString()}"),
                                      subtitle: Text(
                                          "${e['mercaderista']['nombre'].toString()} | ${e['visita']}"),
                                    )))
                                .toList()
                            : <Widget>[
                                const SizedBox(height: 20),
