...
enum status { STARTED, IN_PROGRESS, FINALIZED }
...
class _OrderDetailPageState extends State<OrderDetailPage> {
  ...
  rendering(bool initial_charge) {
    ...
    ciInput.text = (order['sku'][_skuIndex]['inicial'] == null)
        ? ''
        : order['sku'][_skuIndex]['inicial'].toString();
    cfInput.text = (order['sku'][_skuIndex]['final'] == null)
        ? ''
        : order['sku'][_skuIndex]['final'].toString();
    ncInput.text = (order['sku'][_skuIndex]['caras'] == null)
        ? ''
        : order['sku'][_skuIndex]['caras'].toString();
    psInput.text = (order['sku'][_skuIndex]['sugerido'] == null)
        ? ''
        : order['sku'][_skuIndex]['sugerido'].toString();
    obInput.text = (order['sku'][_skuIndex]['observacion'] == null)
        ? ''
        : order['sku'][_skuIndex]['observacion'].toString();
    ppInput.text = (order['sku'][_skuIndex]['participacion'] == null)
        ? ''
        : order['sku'][_skuIndex]['participacion'].toString();
    ccInput.text = (order['sku'][_skuIndex]['cantidad_caducar'] == null)
        ? ''
        : order['sku'][_skuIndex]['cantidad_caducar'].toString();
    caducidadInput.text = (order['sku'][_skuIndex]['proxima_caducidad'] == null)
        ? ''
        : order['sku'][_skuIndex]['proxima_caducidad'].toString();
    setState(() {
      swCenefaPromo = (order['sku'][_skuIndex]['cenefa_promo'] == null)
          ? false
          : order['sku'][_skuIndex]['cenefa_promo'];
      swExhibicion = (order['sku'][_skuIndex]['exhibicion'] == null)
          ? false
          : order['sku'][_skuIndex]['exhibicion'];
    });
  }
  ...
  saveDocument() async {
    ...
    order['sku'][_skuIndex]['inicial'] =
        (ciInput.text.trim() == '') ? '0' : ciInput.text.trim();
    order['sku'][_skuIndex]['final'] =
        (cfInput.text.trim() == '') ? '0' : cfInput.text.trim();
    order['sku'][_skuIndex]['caras'] =
        (ncInput.text.trim() == '') ? '0' : ncInput.text.trim();
    order['sku'][_skuIndex]['sugerido'] =
        (psInput.text.trim() == '') ? '0' : psInput.text.trim();
    order['sku'][_skuIndex]['observacion'] = obInput.text.trim();
    order['sku'][_skuIndex]['participacion'] =
        (ppInput.text.trim() == '') ? '0' : ppInput.text.trim();
    order['sku'][_skuIndex]['cantidad_caducar'] =
        (ccInput.text.trim() == '') ? '0' : ccInput.text.trim();
    order['sku'][_skuIndex]['proxima_caducidad'] =
        (caducidadInput.text.trim() == '') ? '' : caducidadInput.text.trim();
    ...
    if (updated) {
      ...
      if (order['estado']['id'] == 'LT4ytmo1DoCbXR3cj8k2') {
        //STARTED
  ...
  Future uploadFile() async {
    ...
    try {
      ...
      setState(() {
        _list_images.add(
            {'id': _list_images.length + 1, 'nombre': fileName, 'url': url});
      });
    } catch (e) {
      print('error occured');
    }
          ...
          Row(
            ...
            children: <Widget>[
              ...
              Column(
                children: [
                  Text(
                    (order['sku'][_skuIndex]['descripcion'].toString() +
                                    order['sku'][_skuIndex]['presentacion']
                                        .toString())
                                .length >
                            33
                        ? (order['sku'][_skuIndex]['descripcion'].toString() +
                            ...
                            .substring(0, 33)
                        : (order['sku'][_skuIndex]['descripcion'].toString() +
                            order['sku'][_skuIndex]['presentacion'].toString()),
                  ...
                  Text("Sku ${_skuIndex + 1} de ${order['sku'].length}",
                      style: const TextStyle(
                          fontSize: 10,
                          fontWeight: FontWeight.bold,
                          color: Color.fromARGB(255, 14, 77, 128)))
                  ...
                  children: [
                    ...
                    Switch(
                      ...
                      onChanged: (bool value) async {
                        if (value) {
                          if (obInput.text.trim().isEmpty) {
                            await showAlertDialog("Observación requerida",
                                "Antes de bloquear, escribe una observación.");
                            setState(() {
                              swLocked = false;
                            });
                            return;
                ...
                child: Column(children: [
                  ...
                  Padding(
                    ...
                    child: TextButton(
                      ...
                      style: ButtonStyle(
                        backgroundColor:
                            MaterialStateProperty.all<Color>(colorActionCard),
                      ),
                      child: Center(
                        child: Padding(
              ...
              child: Card(
                ...
                child: Padding(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Center(
                        child: Text("${_list_images[index]['nombre']}",
                            style: const TextStyle(
                                color: Colors.black,
                                fontWeight: FontWeight.bold,
                                fontSize: 15))),
              ...
              title: Column(
                children: [
                  ...
                  Center(
                    ...
                    style: const TextStyle(
                        fontSize: 12,
                        color: Color.fromARGB(255, 172, 218, 255)),
                  ))
                ],
              ),
                ...
                children: <Widget>[
                  FloatingActionButton(
                    onPressed: () {
                      ...
                      if (hasUnsaved) {
                        showAlertDialog(
                            "Scope Alert", "No se han guardado todos los skus");
                        return;
                      }
